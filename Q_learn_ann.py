...
from Agent_ann import BaseAgent
...
def q_learning(n_timesteps, learning_rate, gamma, policy='egreedy', epsilon=None, temp=None, plot=True, eval_interval=500):
    ...
    anneal_percentage = 0.2
    start_value = 1.0  # Initial exploration value (epsilon or temperature)
    final_value = 0.1  # Final exploration value (epsilon or temperature)
    ...
    for timestep in range(n_timesteps):
        ...
        # Calculate annealed exploration value
        annealed_value = linear_anneal(timestep, n_timesteps, start_value, final_value, anneal_percentage)

        # Select action using the specified policy
        a = agent.select_action(s, policy, epsilon, temp, annealed_value)
        ...
        # Update reward and move to the next state
        tot_reward += r
        if done:
            s = env.reset()
        else:
